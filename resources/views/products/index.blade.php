...
<div class="sbox-tools" >
	...
	<div class="sbox-content"> 	
	 ...
	 {!! Form::open(array('url'=>'products/delete/', 'class'=>'form-horizontal' ,'id' =>'SximoTable' )) !!}
            ...
            @foreach ($rowData as $row)
                <tr>
				 ...
				 <td>
						...
						<a href="{{ URL::to('products/show/'.$row->id.'?return='.$return)}}" class="tips btn btn-xs btn-white" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search "></i></a>
						@endif
						@if($access['is_edit'] ==1)
						<a  href="{{ URL::to('products/update/'.$row->id.'?return='.$return) }}" class="tips btn btn-xs btn-white" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-edit "></i></a>
						@endif
...
$(document).ready(function(){
	...
	$('.do-quick-search').click(function(){
		$('#SximoTable').attr('action','{{ URL::to("products/multisearch")}}');
		$('#SximoTable').submit();
	});
